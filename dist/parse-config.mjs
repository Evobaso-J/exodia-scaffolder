...
function validateL3Filename(name) {
	if (!L3_FILENAME_RE.test(name)) throw new ConfigError(`l3 filename must match ${L3_FILENAME_RE.source}: '${name}'`);
}
function validateDescription(text) {
	if (typeof text !== "string") throw new ConfigError(`description must be a string, got ${typeName(text)}`);
	if (text === "") throw new ConfigError("description must not be empty");
	if (text.includes("\n") || text.includes("\r")) throw new ConfigError("description must be a single line (no newlines)");
	if (text.length > 200) throw new ConfigError(`description must be <= 200 chars (got ${text.length})`);
}
function validate(parsed) {
	...
	if (rawContextDir !== void 0) if (typeof rawContextDir !== "string" || rawContextDir === "") errors.push(new ConfigError(`context_dir must be a non-empty string, got ${pythonRepr(rawContextDir)}`));
	else try {
	...
	let categoriesMap = {};
	if (rawCategories === void 0 || rawCategories === null) categoriesMap = {};
	else if (!isPlainObject(rawCategories)) errors.push(new ConfigError("categories must be a mapping"));
	else categoriesMap = rawCategories;
	const resolved = [];
	for (const [name, rawBody] of Object.entries(categoriesMap)) {
		if (!CATEGORY_NAME_RE.test(name)) {
			errors.push(new ConfigError(`invalid category name: '${name}' (must match ${CATEGORY_NAME_RE.source})`));
			continue;
		}
		...
		if (drop && (path !== void 0 || customFlag || l3 !== void 0 || description !== void 0)) {
			errors.push(new ConfigError(`category '${name}': 'drop: true' is mutually exclusive with other fields`));
			continue;
		}
		if (!RECOGNIZED_CATEGORIES.has(name) && !customFlag && !drop) {
			const canonicalList = [...RECOGNIZED_CATEGORIES].sort();
			errors.push(new ConfigError(`category '${name}': non-canonical name requires 'custom: true' (canonical set: ${pythonListRepr(canonicalList)})`));
			continue;
		}
		let pathStr;
		if (path !== void 0) {
		...
		if (l3 !== void 0) {
			...
			for (const fname of l3Strs) {
				try {
					...
					if (e instanceof ConfigError) errors.push(new ConfigError(`category '${name}': ${e.message}`));
					else throw e;
				}
				if (seen.has(fname)) errors.push(new ConfigError(`category '${name}': duplicate l3 entry '${fname}'`));
				seen.add(fname);
			}
			l3Override = [...l3Strs];
		}
		let descriptionStr = null;
		if (description !== void 0) try {
	...
	for (const defaultName of CANONICAL_CATEGORIES) if (!declared.has(defaultName)) resolved.push({
		name: defaultName,
	...
	for (const cat of active) {
		const p = cat.path;
		if (seen.has(p)) errors.push(new ConfigError(`categories '${seen.get(p)}' and '${cat.name}' share path '${p}'`));
		else seen.set(p, cat.name);
	}
	const sortedActive = [...active].sort((a, b) => a.path < b.path ? -1 : a.path > b.path ? 1 : 0);
	for (let i = 0; i < sortedActive.length; i++) {
		...
		for (let j = i + 1; j < sortedActive.length; j++) {
			...
			if (other.path.startsWith(`${cat.path}/`)) errors.push(new ConfigError(`category '${cat.name}' path '${cat.path}' is a prefix of category '${other.name}' path '${other.path}'`));
		}
	}
